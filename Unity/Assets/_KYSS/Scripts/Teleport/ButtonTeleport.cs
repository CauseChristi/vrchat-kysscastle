...
public class ButtonTeleport : UdonSharpBehaviour
{
    ...
    [Header("SFX")]
    public AudioSource sfx; // assign your 2D AudioSource here (spatialBlend = 0)
    ...
    private void TryStartTeleport(VRCPlayerApi player)
    {
        ...
        // Play SFX (non-destructive; re-usable)
        if (sfx != null)
        {
            sfx.spatialBlend = 0f; // ensure 2D
            sfx.Stop();
            sfx.time = 0f;
            sfx.Play();
        }
        ...
        if (faceTarget)
        {
            ...
            {
                _queuedRot = target.rotation;
            }
        }
        else
        {
            _queuedRot = player.GetRotation();
        }
